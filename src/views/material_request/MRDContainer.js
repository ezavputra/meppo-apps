...
import MaterialRequestRevisi from './MaterialRequestRevisi';
...
class MRDContainer extends Component {
  constructor(props) {
    super(props);
    this.state = {
      reason: '',
  ...
  async sendApprove(mode) {
    ...
    try {
      ...
      if (mode == 'Reject') {
        ...
        response = await axios.post("/material_request/reject/" + params.item.id, formData, {
          headers: {
      ...
      if (mode == 'Approve') {
        response = await axios.post("/material_request/approve/" + params.item.id, formData, {
          headers: {
            accept: "multipart/form-data"
          }
        });
          ...
          <Box flex={1}>
            ...
            <TabView
              ...
              renderScene={SceneMap({
                info: () => <MaterialRequestInfo navigation={navigation} params={params} />,
                detail: () => <MaterialRequestDetail navigation={navigation} params={params} />,
                revisi: () => <MaterialRequestRevisi navigation={navigation} params={params} />,
              })}
...
export default connect(mapStateToProps, mapDispatchToProps)(MRDContainer);