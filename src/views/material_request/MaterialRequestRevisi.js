...
class MaterialRequestRevisi extends Component {
  constructor(props) {
    super(props);
    this.state = {
      tabIndex: 0,
  ...
  async fetchData() {
    ...
    try {
      const response = await axios.post("/material_request_revisi/" + params.item.id);
      let step = [];
...
MaterialRequestRevisi.propTypes = {
...
export default connect(mapStateToProps, mapDispatchToProps)(MaterialRequestRevisi);